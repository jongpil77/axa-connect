...
// 생일 섹션 타이틀 크기 조정 (text-lg -> text-sm) 및 높이 대응
...
const HomeTab = ({ mood, handleMoodCheck, handleCheckOut, hasCheckedOut, feeds, onWriteClickWithCategory, onNavigateToNews, onNavigateToFeed, weeklyBirthdays, boosterActive }) => {
    ...
    return (
      <div className="p-5 space-y-5 pb-32 animate-fade-in relative bg-blue-50 min-h-full">
        {/* 1. 공지사항 섹션 (맨 위로 이동 + 글쓰기 버튼 추가) */}
        <div>
           {/* 글쓰기 버튼 & 타이틀 헤더 */}
           <div className="flex justify-between items-end mb-3 px-1">
              <div className="flex items-center gap-2">
                 {/* 공지사항 상단 좌측 글쓰기 버튼: null을 전달하여 카테고리 선택 가능하게 함 */}
                 <button onClick={() => onWriteClickWithCategory(null)} className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-3 py-1.5 rounded-xl text-xs font-bold shadow-md flex items-center gap-1 active:scale-95 transition-all">
                    <span>➕</span> 글쓰기
                 </button>
        ...
        {/* 2. 출퇴근 체크 & 생일자 섹션 (공지사항 아래로 이동, 생일자 섹션 높이 h-56으로 키움) */}
  ...
  return (
    <div className="p-5 space-y-5 pb-28 animate-fade-in bg-blue-50">
      ...
      <div className="flex flex-col items-end gap-1 mb-1">
          {/* 소통 탭 내에서도 글쓰기 버튼 클릭 시 카테고리 선택(null 전달) 가능하도록 변경 */}
...
const WriteModal = ({ setShowWriteModal, handlePostSubmit, currentUser, activeTab, boosterActive, initialCategory }) => {
  ...
  useEffect(() => {
      // initialCategory가 null이거나 없는 경우(글쓰기 버튼 클릭), 기본적으로 첫 번째 카테고리를 선택
...
export default function App() {
  ...
  // 탭 변경 핸들러 (소통 탭 클릭 시 필터 초기화 로직 추가)
