    ...
    <div className="bg-white/80 backdrop-blur-md p-4 sticky top-0 z-30 border-b border-slate-100 shadow-sm">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 relative">
          <button 
            onClick={onOpenGift} 
            className="p-1.5 rounded-full hover:bg-slate-100 active:scale-95 transition-all relative"
          >
          ...
          >
             ...
             <div className="flex items-center gap-1">
                 ...
                 <span className="text-lg font-black text-blue-700 animate-pulse">{currentUser?.points?.toLocaleString()} P</span>
             </div>
          </div>
        ...
        <div>
           <div className="flex justify-between items-end mb-3 px-1">
               ...
               <div className="flex items-center gap-2">
                   {/* 새 글쓰기 버튼 위치 이동 및 안내 문구 추가 */}
                   <div className="flex items-center gap-1.5 text-[9px] font-bold text-slate-500 bg-white px-2 py-1 rounded-full shadow-sm border border-slate-100 mr-1">
                        <div className="w-3.5 h-3.5 bg-yellow-400 rounded-full flex items-center justify-center shadow-inner"><Coins className="w-2 h-2 text-white fill-white"/></div>
                        게시글 1개 +50P (일 최대 +100P)
                   </div>
                   <button onClick={() => onWriteClickWithCategory(null)} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-md hover:bg-slate-700 transition-colors flex items-center gap-1"><Pencil className="w-3 h-3"/>글쓰기</button>
                   <button onClick={onNavigateToNews} className="text-xs text-slate-400 font-medium hover:text-blue-600 flex items-center gap-0.5 ml-1">더보기 <ChevronRight className="w-3 h-3" /></button>
               </div>
           </div>
...
// BottomNav - 높이 확대, 아이콘 확대, 텍스트 가로 배치 (수정됨)
        ...
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 w-[90%] max-w-[360px] bg-[#00008F] backdrop-blur-md border border-blue-900 shadow-[0_8px_30px_rgb(0,0,0,0.3)] p-1.5 z-30 flex justify-between items-center rounded-full h-20">
            {[{ id: 'home', icon: User, label: '홈' }, { id: 'feed', icon: MessageCircle, label: '소통' }, { id: 'news', icon: Bell, label: '소식' }, { id: 'ranking', icon: Award, label: '랭킹' }].map(item => (
                <button 
                    ...
                    className={`flex-1 flex flex-row items-center justify-center gap-1.5 py-2.5 rounded-full transition-all duration-300 ${getTabColor(item.id, activeTab === item.id)}`}
                >
                    <item.icon className={`w-7 h-7 ${activeTab === item.id ? 'stroke-[2.5px]' : ''}`} />
