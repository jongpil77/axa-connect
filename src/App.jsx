          ...
          <div className="space-y-3 flex flex-col">
            <form onSubmit={handleLogin} className="space-y-2">
              <div>
                  ...
                  <input name="email" type="text" placeholder="이메일 입력" className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg outline-none text-[11px] focus:border-blue-500 focus:bg-white transition-all shadow-sm" />
              </div>
              <div>
                  <label className="block text-[10px] font-bold text-slate-600 mb-0.5 ml-1">비밀번호</label>
                  <input name="password" type="password" placeholder="비밀번호 (숫자 6자리 이상)" className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg outline-none text-[11px] focus:border-blue-500 focus:bg-white transition-all shadow-sm" required minLength="6" />
              </div>
              <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white p-2 rounded-lg text-[11px] font-bold hover:bg-blue-700 shadow-md transition-all active:scale-[0.98] disabled:bg-blue-300 flex justify-center mt-1">
                  {loading ? <Loader2 className="animate-spin w-4 h-4" /> : '🚀 로그인'}
              </button>
            </form>
            <div className="text-center pt-1">
                <button onClick={() => setIsSignupMode(true)} className="text-slate-500 text-[11px] font-bold hover:text-blue-600 underline transition-colors">
                    회원 가입
                </button>
            </div>
          </div>
